<div class="row container-fluid ">
		<div class="col-sm-3 bg-secondary sidelist">
			<h6><a class="listtabel" href="/sisjamtu/daftartabel">Tabel Program Studi</a></h6>
			<h6><a class="listtabel" href="/sisjamtu/daftartabel/kerjasamapendidikan">Tabel Kerjasama Pendidikan</a></h6>
			<h6><a class="listtabel" href="/sisjamtu/daftartabel/kerjasamapenelitian">Tabel Kerjasama Penelitian</a></h6>
			<h6><a class="listtabel" href="/sisjamtu/daftartabel/kerjasamapengabdian">Tabel Kerjasama Pengabdian Masyarakat</a></h6>
			<h6><a class="listtabel" href="/sisjamtu/daftartabel/seleksimhs">Tabel Seleksi Mahasiswa Baru</a></h6>
			<h6><a class="listtabel" href="/sisjamtu/daftartabel/mahasiswaasing">Tabel Mahasiswa Asing</a></h6>
			<h6><a class="listtabel" href="/sisjamtu/daftartabel/dosentetap">Tabel Dosen Tetap Perguruan Tinggi</a></h6>
			<h6><a class="listtabel" href="#">Tabel Dosen Pembimbing Utama Tugas Akhir</a></h6>
			<h6><a class="listtabel" href="#">Tabel Ekuivalen Waktu Mengajar Penuh Dosen Tetap Perguruan Tinggi</a></h6>
			<h6><a class="listtabel" href="#">Tabel Dosen Tidak Tetap</a></h6>
			<h6><a class="listtabel" href="#">Tabel Industri/Praktisi</a></h6>
			<h6><a class="listtabel" href="#">Tabel Pengakuan/Rekognisi Dosen</a></h6>
			<h6><a class="listtabel" href="#">Tabel Penelitian DTPS</a></h6>
			<h6><a class="listtabel" href="#">Tabel PkM DTPS</a></h6>
			<h6><a class="listtabel" href="#">Tabel Publikasi Ilmiah DTPS</a></h6>
			<h6><a class="listtabel" href="#">Tabel Pagelaran/Pameran/Presentasi/Publikasi Ilmiah DTPS</a></h6>
			<h6><a class="listtabel" href="#">Tabel Luaran Penelitian/PkM Lainnya-HKI (Paten, Paten Sederhana)</a></h6>
			<h6><a class="listtabel" href="#">Tabel Luaran Penelitian/PkM Lainnya - HKI (Hak Cipta, Desain Produk Industri, dll.)</a></h6>
			<h6><a class="listtabel" href="#">Tabel Luaran Penelitian/PkM Lainnya - Teknologi Tepat Guna, Produk, Karya Seni, Rekayasa Sosial</a></h6>
			<h6><a class="listtabel" href="#">Tabel Luaran Penelitian/PkM Lainnya - Buku ber-ISBN, Book Chapter</a></h6>
			<h6><a class="listtabel" href="#">Tabel Karya Ilmiah DTPS yang Disitasi</a></h6>
			<h6><a class="listtabel" href="#">Tabel Produk/Jasa DTPS yang Diadopsi oleh Industri/Masyarakat</a></h6>
		</div>